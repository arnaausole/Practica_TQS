...
class DealerTest {
    ...
    // LOOP TESTING: BUCLE SIMPLE 'while' a Dealer.play()
    @Test
    void testPlay_LoopTesting_While() {
        Dealer dealer = new Dealer();

        // Caixa Blanca: Loop Testing (0 vegades)
        // Condició d'entrada falsa (hand.getValue() >= 17). No entra al 'while'.
        dealer.getHand().addCard(new Card("Hearts", "10"));
        dealer.getHand().addCard(new Card("Clubs", "7")); // 17
        
        MockDeck mock0 = new MockDeck(); 
        int initialCards0 = dealer.getHand().getCards().size();
        
        dealer.play(mock0);
        
        assertEquals(17, dealer.getValue(), "Caixa Blanca (Loop 0): No ha de robar si té 17.");
        assertEquals(initialCards0, dealer.getHand().getCards().size(), "Caixa Blanca (Loop 0): No s'ha d'afegir cap carta.");


        // Caixa Blanca: Loop Testing (1 vegada)
        // Entra al 'while' una vegada, roba una carta i el valor passa a ser >= 17.
        dealer.reset();
        dealer.getHand().addCard(new Card("Hearts", "10"));
        dealer.getHand().addCard(new Card("Clubs", "6")); // 16. Entra.
        
        MockDeck mock1 = new MockDeck();
        mock1.setCards(new Card("Spades", "2")); // Roba un 2. Total 18. Surt.
        
        dealer.play(mock1);
        
        assertEquals(18, dealer.getValue(), "Caixa Blanca (Loop 1): Ha de robar una sola carta.");
        assertEquals(3, dealer.getHand().getCards().size(), "Caixa Blanca (Loop 1): S'ha d'afegir 1 carta.");


        // Caixa Blanca: Loop Testing (2+ vegades)
        // Entra al 'while' diverses vegades fins assolir >= 17.
        dealer.reset();
        dealer.getHand().addCard(new Card("Hearts", "9")); // Total 9.
        
        MockDeck mock2 = new MockDeck();
        mock2.setCards(
            new Card("Spades", "4"),  // 1a volta: 9+4=13. Entra.
            new Card("Diamonds", "4") // 2a volta: 13+4=17. Surt.
        );
        
        dealer.play(mock2);
        
        assertEquals(17, dealer.getValue(), "Caixa Blanca (Loop 2+): Ha de robar 2 cartes.");
        assertEquals(3, dealer.getHand().getCards().size(), "Caixa Blanca (Loop 2+): S'han d'afegir 2 cartes.");
    }
}
